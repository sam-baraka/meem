...
// The scale of the child at the time that the CustomContextMenu opens.
...
/// A function that produces the preview when the CustomContextMenu is open.
///
...
// on the screen of [CustomContextMenu.child] before the
// [CustomContextMenu] opens.
...
///
/// When open, the [CustomContextMenu] shows the child, or the widget returned
...
/// unconstrained.
///
/// When closed, the CustomContextMenu simply displays the child as if the
/// CustomContextMenu were not there. Sizing and positioning is unaffected.
...
///
/// This sample shows a very simple CustomContextMenu for an empty red
...
class CustomContextMenu extends StatefulWidget {
  ...
  /// The widget that can be "opened" with the [CustomContextMenu].
  ///
  /// When the [CustomContextMenu] is long-pressed, the menu will open and
  ...
  /// doesn't size itself.
  ///
  /// When the [CustomContextMenu] is "closed", this widget acts like a
  ...
  /// the CustomContextMenu opens, and from 1 to 0 when it closes, and it can
  ...
  /// rounded corners. The larger size of the open CustomContextMenu allows
  ...
  /// into view as the CustomContextMenu is opened. The preview is swapped in
  ...
  ///   // aspect ratio when the CustomContextMenu is animating its preview
  ///   // widget open and closed.
  ///   previewBuilder: (BuildContext context, Animation<double> animation, Widget child) {
  ///     return FittedBox(
  ///       fit: BoxFit.cover,
  ///       // This ClipRRect rounds the corners of the image when the
  ///       // CustomContextMenu is open, even though it's not rounded when
  ...
  @override
  _CustomContextMenuState createState() => _CustomContextMenuState();
}
...
class _CustomContextMenuState extends State<CustomContextMenu>
  ...
  // align the content of the open CustomContextMenu. For example, if the
  ...
  // Push the new route and open the CustomContextMenu overlay.
  ...
  // the CustomContextMenu just behaves as a Container.
...
// A floating copy of the CustomContextMenu's child.
//
// When the child is pressed, but before the CustomContextMenu opens, it does
...
// The open CustomContextMenu modal.
class _ContextMenuRoute<T> extends PopupRoute<T> {
  ...
  // Barrier color for a Custom modal barrier.
  ...
  // The Rect of the child at the moment that the CustomContextMenu opens.
...
class _ContextMenuRouteStaticState extends State<_ContextMenuRouteStatic>
  ...
  // The CustomContextMenuSheet disappears at this scale.
...
// The menu that displays when CustomContextMenu is open. It consists of a
// list of actions that are typically CustomContextMenuActions.
